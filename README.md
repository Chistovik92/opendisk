...
телефонов, `-arm32.apk` для приставок Android TV и часов Wear OS,
`-universal.apk` — если не встал ни один.

С 0.5.6 внутри — **свой файловый менеджер**: облака, память телефона, карты
памяти и (на телефонах с root) вся файловая система на одной вкладке
«Диски»; файлы открываются, копируются и переносятся между ними, как в
обычном файловом менеджере. Облака добавляются на вкладке «Добавить».
Приложение попросит доступ ко всем файлам — без него память телефона не видна.

С 0.5.4 облако добавляется **входом через браузер** — Google Диск,
OneDrive, Dropbox, Яндекс.Диск и все остальные, кто так умеет: выбрать
аккаунт и разрешить доступ, без паролей. Каталог сервисов тот же, что на
компьютере. С 0.5.3 облако можно **подключить**: переключатель в списке — и оно
