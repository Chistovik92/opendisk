...
приложение, и обновление поверх запущенной копии упиралось в занятые файлы —
Windows Installer сам перезагружал компьютер, а установка оставалась
повреждённой. В своём описании есть `util:CloseApplication`, который закрывает
приложение до того, как трогать файлы.

Инструменты WiX отдельно ставить не нужно: их приносит задача `unzipWix`
плагина Compose.

Три грабли, на которые здесь легко наступить снова:

- **GUID компонентов должны быть стабильными** (`heat -ag`, а не `-gg`). Со
  случайными GUID-ами installer не понимает, что файлы старой и новой версии —
  одни и те же: ссылки не пересчитываются, и удаление старой версии сносит
  только что скопированное. При одном порядке действий пропадали библиотеки
  JVM, при другом — почти весь образ.
- **`light` нужно указывать культуру** (`-cultures:ru-ru`), иначе он собирает
  базу в кодовой странице 1252 и падает на любой кириллице.
- **ICE38, ICE43 и ICE57 подавлены осознанно**: они считают компоненты
  с ярлыками пользовательскими и требуют ключ в HKCU. Установка машинная,
  каталоги общие — претензия не по адресу. Что ярлыки ставятся и снимаются
  правильно, проверено установкой.
...
Оставлен как запасной вариант, но обновление поверх запущенного приложения им
делать нельзя.
