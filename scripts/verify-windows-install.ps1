...
param(
    ...
    # Выключается там, где это заведомо не проверить. Завели на ARM-раннере,
    # пока казалось, что там не стартует ничего с окнами; проба запуска
    # показала, что висел только лаунчер 0.5.0.
